...
export default function MortgageCalculator() {
  ...
  const calculateMortgage = () => {
    ...
    try {
      ...
      if ([price, down, rate, term, tax, ins].some(isNaN)) {
        throw new Error('Please enter valid numbers for all fields');
      }

      if (price <= 0) {
        throw new Error('Home price must be greater than zero');
      }
      if (down < 0 || down >= price) {
        throw new Error('Down payment must be between 0 and home price');
      }
      if (rate <= 0 || rate >= 100) {
      ...
      if (tax < 0) {
        throw new Error('Property tax cannot be negative');
      }
      if (ins < 0) {
        throw new Error('Insurance cannot be negative');
      }
      ...
      const periodTax = tax / paymentsPerYear;
      const periodInsurance = ins / paymentsPerYear;
      const totalPeriodic = payment + periodTax + periodInsurance;
      
      const totalPrincipalPaid = payment * totalPayments;
      const totalInterest = Math.max(0, totalPrincipalPaid - principal);
      const downPaymentPercentage = (down / price) * 100;

      setResults({
        payment: Number(payment.toFixed(2)),
        totalInterest: Number(totalInterest.toFixed(2)),
        totalCost: Number((totalInterest + price).toFixed(2)),
        periodTax: Number(periodTax.toFixed(2)),
        periodInsurance: Number(periodInsurance.toFixed(2)),
        totalPeriodic: Number(totalPeriodic.toFixed(2)),
        downPaymentPercentage: Number(downPaymentPercentage.toFixed(2)),
          ...
          <div>
            ...
            <div className="prose max-w-none mb-6">
              <p className="text-gray-600">
                Calculate your estimated mortgage payments and understand the total cost of homeownership.
                This calculator helps you:
              </p>
              <ul className="space-y-2 text-gray-600 list-none pl-0">
                {[
                  'Estimate payments for different frequencies',
          ...
          <div className="space-y-6">
            <Card className="bg-white/80 backdrop-blur-sm border-purple-100">
              <div className="p-6">
                  ...
                  <div className="space-y-6">
                    <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg p-4 text-white">
                      <p className="text-sm opacity-90 mb-1">
                        {results.paymentFrequencyLabel.charAt(0).toUpperCase() + results.paymentFrequencyLabel.slice(1)} Payment (P&I)
                      </p>
                    ...
                    <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg p-4 text-white">
                      <p className="text-sm opacity-90 mb-1">Total {results.paymentFrequencyLabel} Payment</p>
                      <p className="text-3xl font-bold">
                        ${results.totalPeriodic.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2
                        })}
                      </p>
                      <div className="mt-2 space-y-1 text-sm opacity-90">
                        <p>Includes:</p>
                        <p>Property Tax: ${results.periodTax.toLocaleString(undefined, { minimumFractionDigits: 2 })}/{results.paymentFrequencyLabel}</p>
                        <p>Insurance: ${results.periodInsurance.toLocaleString(undefined, { minimumFractionDigits: 2 })}/{results.paymentFrequencyLabel}</p>
                      </div>
                    </div>

                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="p-4 bg-purple-50 rounded-lg">
                        <p className="text-sm text-indigo-600 font-medium mb-1">Down Payment</p>
                        <p className="text-lg font-bold text-indigo-600">
                          {results.downPaymentPercentage.toFixed(1)}%
                        </p>
                      </div>
                      <div className="p-4 bg-purple-50 rounded-lg">
                ...
                <div className="mt-6 space-y-4">
                  <h3 className="font-semibold text-indigo-600">Understanding Your Mortgage:</h3>
                  <ul className="space-y-2">
                    {[
                      'Payment frequency affects total interest paid',
                      'More frequent payments can reduce loan term',
                      'Down payment affects loan amount and PMI',
                      'Consider the impact of payment schedule on your budget'
                    ].map((item, index) => (
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-purple-100">
              <div className="p-6">
                ...
                <ul className="space-y-2">
                  {[
                    'Weekly payments can help reduce total interest',
                    'Choose a frequency that matches your income schedule',
                    'More frequent payments build equity faster',
                    'Consider automatic payments to stay on schedule',
                    'Review your budget to select the best frequency'
                  ].map((tip, index) => (
                    <li key={index} className="flex items-center space-x-2 text-gray-600">
                      <span className="h-1.5 w-1.5 rounded-full bg-purple-600 flex-shrink-0" />
